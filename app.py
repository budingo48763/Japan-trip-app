...
# 判斷作業系統，若是 Linux (Streamlit Cloud) 則嘗試設定 CJK 字體
system_platform = platform.system()
if system_platform == "Windows":
    plt.rcParams['font.sans-serif'] = ['Microsoft JhengHei']
else:
    # 針對 Linux/Streamlit Cloud 環境設定字體
    # 必須配合 packages.txt 安裝 fonts-noto-cjk
    plt.rcParams['font.family'] = ['sans-serif']
    plt.rcParams['font.sans-serif'] = ['Noto Sans CJK JP', 'Noto Sans CJK TC', 'WenQuanYi Zen Hei', 'DejaVu Sans']
...
# 注意：在 Linux 上指定 Windows 字體可能會失效，這裡移除 fontname 強制設定，讓系統自動抓取
...
if st.button("🎨 產生行程圖"):
    ...
    if items:
        ...
        trip_flow.attr('node', shape='box', style='filled', color='lightblue') # 移除 fontname 避免亂碼
...
for i, item in enumerate(sorted_items):
    ...
    flow.node(str(i), label, style='filled', color=style["color"]) # 移除 fontname
    if i > 0:
