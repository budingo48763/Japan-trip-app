...
st.markdown(f"""
    ...
    /* 行程卡片設計 */
    .app-card {{
        background: {THEME['card']}; border-radius: 16px; padding: 16px;
    ...
    /* 導航按鈕 */
    .nav-btn {{
        background: #fff; color: {THEME['primary']}; border: 1px solid {THEME['primary']};
        padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; text-decoration: none; display: inline-block;
    }}
    ...
    .guide-label {{
        font-weight: bold; color: #ff6f00; margin-right: 5px;
    ...
    .ai-tag {{
        ...
        font-size: 0.75rem; color: white; margin-right: 4px; font-weight: bold;
    }}

    /* 輸入框優化 */
    .stTextInput input, .stNumberInput input {{
        background: {THEME['card']}; border-radius: 8px;
    }}
    </style>
""", unsafe_allow_html=True)

# -------------------------------------
# 2. 模擬導遊資料庫 (AI Guide Logic)
# -------------------------------------
# 這裡模擬「根據地點自動分析」的功能。實際開發可接 OpenAI API。
GUIDE_DB = {
    "Outlet": {
    ...
    "清水寺": {
        "story": "京都最古老寺院，著名的「清水舞台」沒有使用任何一根釘子。",
        "food": "瀧之家 湯豆腐、藤菜美 糯米糰子。",
        "buy": "清水燒陶瓷、西尾八橋 (生八橋)、御守"
    },
    "租車": {
...
def get_ai_guide(title, location):
    """簡單的關鍵字匹配，模擬 AI 導遊建議"""
    search_text = (title + location).lower()
...
def auto_highlight_text(text):
    """關鍵字高亮"""
    if not text: return ""
    text = re.sub(r'(必吃|推薦|名物)', r'<span class="ai-tag" style="background:#FF8C42">🍱 \1</span>', text)
    text = re.sub(r'(必買|伴手禮|限定)', r'<span class="ai-tag" style="background:#E63946">🛍️ \1</span>', text)
    text = re.sub(r'(預約|代號)', r'<span class="ai-tag" style="background:#2A9D8F">🎫 \1</span>', text)
    return text
...
with tab_schedule:
    ...
    for i, item in enumerate(day_items):
        ...
        guide_html = ""
        if guide_info and not is_edit: # 編輯模式時不顯示導遊，避免雜亂
            guide_html = f"""
            <div class="guide-box">
        ...
        note_html = f"<div style='margin-top:8px; color:#666; font-size:0.9rem;'>{auto_highlight_text(item['note'])}</div>" if item['note'] else ""
...
with tab_info:
    ...
    # 4. 行李清單 (原本的獨立功能，現已移入此處)
    with st.expander("🧳 行李檢查清單", expanded=False):
        st.caption("出發前請再次確認")
